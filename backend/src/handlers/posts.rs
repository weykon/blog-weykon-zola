...
pub async fn list(
    ...
    // Query total count (only posts, not mutters)
    let total: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM posts WHERE content_type = 'post' AND is_draft = false"
    )
    .fetch_one(&state.db)
    .await
    .unwrap_or((0,));
    ...
    // Query posts from database (only posts, not mutters)
...
pub async fn detail(
    ...
    // Query post by slug (only posts, not mutters)
    let post = sqlx::query_as::<_, crate::models::Post>(
        "SELECT * FROM posts WHERE slug = $1 AND content_type = 'post' AND is_draft = false"
    )
    .bind(&slug)
...
pub async fn by_tag(
    ...
    // Get posts by tag (only posts, not mutters)
    let posts = sqlx::query_as::<_, crate::models::Post>(
        "SELECT p.* FROM posts p
         JOIN post_tags pt ON p.id = pt.post_id
         JOIN tags t ON pt.tag_id = t.id
         WHERE t.slug = $1 AND p.content_type = 'post' AND p.is_draft = false
